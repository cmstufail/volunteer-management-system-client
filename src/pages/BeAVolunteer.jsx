...
const BeAVolunteer = () => {
    ...
    return (
        <Container>
            <div className="bg-base-200 py-10 p-8 pt-24">
                <div className="p-8 bg-base-100 shadow-xl rounded-lg">
                    ...
                    <h3 className="text-xl font-semibold mb-8 text-center text-primary">{ post.postTitle }</h3>
                    <form onSubmit={ handleSubmitRequest } className="space-y-4">
                        {/* Read-only Post Information */ }
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4"> 
                            <input type="text" defaultValue={ `Category: ${ post.category }` } readOnly disabled className={ `mt-2 ${ isDark ? 'text-gray-300' : 'text-gray-600' }input input-bordered w-full` } />
                            <input type="text" defaultValue={ `Deadline: ${ new Date( post.deadline ).toLocaleDateString() }` } readOnly disabled className={ `mt-2 ${ isDark ? 'text-gray-300' : 'text-gray-600' }input input-bordered w-full` } />
                        </div>

                        {/* Read-only User Information */ }
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4 border-t mt-4">
                            <input type="text" defaultValue={ `Your Name: ${ user.displayName }` } readOnly disabled className={ `mt-2 ${ isDark ? 'text-gray-300' : 'text-gray-600' }input input-bordered w-full` } />
                            <input type="email" defaultValue={ `Your Email: ${ user.email }` } readOnly disabled className={ `mt-2 ${ isDark ? 'text-gray-300' : 'text-gray-600' }input input-bordered w-full` } />
                        </div>

                        {/* Editable Fields */ }
                        <div className="form-control my-16">
                            <label className={ `mt-2 ${ isDark ? 'text-gray-300' : 'text-gray-600' }` }><span className="label-text">Suggestion / Motivation</span></label>
                            <textarea
                                value={ suggestion }
