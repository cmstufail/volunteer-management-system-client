...
const Navbar = () => {
    ...
    return (
        <Container>
            <nav className={ `transition-colors duration-300 fixed w-full z-50 shadow-sm py-1 top-0 left-0 right-0 ${ theme === "dark" ? "bg-gray-900 text-gray-300" : "bg-base-100 text-gray-900" }` }>
                <div className="container mx-auto flex justify-between items-center px-4">
                    {/* Logo */ }
                    <div className="flex items-center">
                        <Link to="/" className="flex items-center gap-2">
                            <img src={ theme === 'dark' ? logoBlack : logoWhite } className="w-10 h-10" alt="Logo" />
                    ...
                    <div className="flex items-center gap-2">
                        <button onClick={ toggleTheme } className="btn btn-ghost btn-circle">
                            { theme === "dark" ? <FaSun className="text-yellow-400 text-xl" /> : <FaMoon className="text-xl" /> }
                        </button>
                        { !user ? (
                            ...
                            <div className="dropdown dropdown-end dropdown-hover hidden md:flex">
                                ...
                                <ul tabIndex={ 0 } className="dropdown-content menu p-2 shadow bg-base-100 rounded-box w-52 mt-3 z-[1] border dark:border-gray-700 mt-12 ml-10">
                                    <li className="px-4 py-2 font-bold border-b dark:border-gray-600 mb-1">{ user.displayName }</li>
                                    <li><button onClick={ handleLogout } className="w-full text-left p-2 hover:bg-base-200 rounded text-red-500">Logout</button></li>
                                </ul>
                            </div>
                        ) }
                        ...
                        <div className="flex flex-col gap-2">
                            ...
                            { user && (
                                <>
                                    <div className="flex items-center gap-3 p-2 rounded-lg bg-base-200 mb-2">
                                        <div className="avatar"><div className="w-10 rounded-full"><img src={ user.photoURL || 'https://i.pravatar.cc/150' } alt={ user.displayName } /></div></div>
                                        <span className="font-bold">{ user.displayName }</span>
                                    </div>
                                    <details className="collapse collapse-arrow bg-base-200">
                                        <summary className="collapse-title font-medium">My Profile</summary>
                                        <div className="collapse-content">
                                            <ul className="menu menu-compact">
                                                <li><NavLink to="/add-post" onClick={ closeAllMenus }>Add Volunteer Post</NavLink></li>
                                                <li><NavLink to="/manage-posts" onClick={ closeAllMenus }>Manage My Posts</NavLink></li>
                                            </ul>
                                        </div>
                                    </details>
                                    <NavLink to="/my-volunteer-list" onClick={ closeAllMenus } className={ getNavLinkClass }>My Applied Posts</NavLink>
                                    <div className="divider my-2"></div>
                                </>
                            ) }
                            <div className="flex flex-col gap-2 mt-2">
                                { user ? (
                                    ...
                                    <>
                                        <Link to="/login" onClick={ closeAllMenus } className="btn btn-primary w-full">Login</Link>
                                        <Link to="/register" onClick={ closeAllMenus } className="btn btn-secondary w-full">Register</Link>
                                    </>
                                ) }
                            </div>
                        </div>
